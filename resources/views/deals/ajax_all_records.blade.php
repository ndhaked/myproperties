...
<div class="bg-white rounded-lg border border-gray-100">
    <div class="deals-grid artist-list-grid artist-list-header-grid grid items-center px-2 border-gray-100 bg-[#FAFAFA] text-xs font-semibold text-gray-500 uppercase tracking-wider">
        ...
        <span class="text-left list-heading-text">PROJECT NAME</span>
        <span class="text-left list-heading-text">PROJECT TYPE</span>
    ...
    <div id="artwork-list">
       @forelse($records as $key => $recordList)
           <div class="deals-grid artist-list-grid artist-list-header-grid grid items-center px-2 border-t border-gray-100 transition duration-100 hover:bg-[#FAFAFA]" data-gallery-id="modern-art">
               <input type="checkbox" class="w-0 h-0 text-violet-600 border-gray-300 rounded focus:ring-violet-500">
               <span class="list-row-text text-left art-gallery-name">{{$recordList->id}}</span>
               <span class="list-row-text text-left art-gallery-name">{{ucwords($recordList->client_name)}}</span>
               <!-- <span class="list-row-text text-left art-gallery-name">{{$recordList->client_mobile_no}}</span> -->
               <span class="list-row-text text-left art-gallery-name">{{$recordList->project->name}}</span>
               <span class="list-row-text text-left art-gallery-name">{{$recordList->projectType->name}}</span>
               ...
               <span class="list-row-text text-center">
                   <span class="list-status-published status_{{$recordList->status}}">
                       <span class="dot"></span>
                       {{ucfirst($recordList->dealStatus->name)}}
                   </span>
               </span>
               <div class="relative flex justify-end">
                    <button class="action-menu-btn p-1 rounded-full row-button">
                        ...
                        <div class="action-menu-dropdown absolute right-4 w-full top-full bg-white shadow-xl z-10  hidden">
                            <a href="{{route('deals.edit',$recordList->id)}}" class="dot-menu-button">Edit Deal</a>
                            <a href="javascript:;" class="dot-menu-button deleteArtist" data-id="{{ $recordList->id }}">Delete Deal</a>
                   </div>
               </div>
           </div>
       @empty
